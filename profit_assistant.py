...
LEVERS: List[Tuple[str, str, str]] = [
    ...
    (
        "WALLET_DEPLOY_PCT",
        "0.80",
        "Deploy fraction × collateral; budget × LEVERAGE = max notional (.env.example uses 0.65 for headroom)",
    ),
    ...
    (
        "MAX_POSITIONS_AUTO",
        "false",
        "true = fewer concurrent slots when deploy_cap is small (cap still MAX_POSITIONS)",
    ),
    ("MIN_SLOT_USD", "", "With AUTO: divisor floor (default = WALLET_MIN_USD)"),
    (
        "ALLOCATION_MODE",
        "rank_weighted",
        "rank_weighted | equal | funding_linear — how new-leg budget is split",
    ),
    ("RESIZE_OPEN_LONGS", "false", "Live Aster-only: add notional when under deploy target (not DRY_RUN/HL)"),
    ("RESIZE_MIN_INTERVAL_SEC", "3600", "Min seconds between resize adds per symbol"),
    ("RESIZE_MIN_DELTA_USD", "25", "Min USD gap (target − live) before a resize buy"),
    ("RESIZE_BAND_PCT", "0.02", "Also require gap ≥ this fraction of target notional"),
    (
        "RESERVE_SLOT_FOR_NEW_POOLS",
        "false",
        "true = ~1/MAX_POSITIONS of deploy cap unused for new pool names; false = full cap fillable",
    ),
    ...
    (
        "STOP_LOSS_PCT",
        "0.05",
        "Close if mark vs entry adverse by this fraction (.env.example uses 0.10 vs thin-mark whipsaw)",
    ),
    ...
    ("BLACKLIST", "", "Never trade; .env.example includes example illiquid micro-cap names"),
    (
        "MIN_QUOTE_VOLUME_24H",
        "0",
        "Min 24h USDT quote vol; 0=off (.env.example / fly.toml use 5e6 for pool quality)",
    ),
